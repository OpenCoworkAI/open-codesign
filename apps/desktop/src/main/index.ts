...
function registerIpcHandlers(db: Database | null): void {
  ...
  /**
   * Lookup the filesystem workspace bound to a design. Reads the
   * `workspace_path` column added by PR #173; returns undefined when the
   * design has no workspace attached or the DB is unavailable (so core's
   * scaffold tool surfaces "no workspace attached" rather than writing to
   * a guessed path).
   */
  const resolveWorkspaceRootForDesign = (designId: string | null): string | undefined => {
