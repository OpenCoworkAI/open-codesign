...
Synced with upstream `3b03e7a` (#430 local React source editing) via normal merge commit `3ca7e72`, preserving published history, current question/run scope and both runtime regression suites. The normal pre-commit and pre-push hooks were used, not bypassed.

| Check | Result |
| --- | --- |
| `pnpm lint` | Pass |
| `pnpm -r typecheck` (normal pre-push hook) | Pass across the workspace |
| `pnpm test --concurrency=1` and normal pre-push `pnpm test` | 3,614 Vitest tests passed, 4 skipped; 61 CI-tool script tests passed |
...
- `03532a4` — bot follow-up: guidance gating, primary-export resilience and read-only records.
- `3ca7e72` — merge latest main, preserving upstream source-edit and script-literal behavior.
...
- [x] Lean: one lazy HTML5 parser dependency (cost/alternatives below); no bundled browser/model/Python runtime, provider SDK, hosted service layer, or research panel.
