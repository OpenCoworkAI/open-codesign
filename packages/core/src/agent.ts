...
const AGENTIC_TOOL_GUIDANCE = [
  ...
  '',
  '**CRITICAL: Do NOT write a full HTML document.** Never emit `<!doctype html>`, `<html>`, `<head>`, `<body>`, or `<script src="...">` tags. The host injects those. Writing a full HTML page is a hard failure — it breaks the live preview and wastes your entire context budget.',
  '',
  ...
  '',
  '### Required cadence',
  '1. **First turn — plan + skeleton.** In a SINGLE turn: (a) call `set_todos` with **7–10 checklist items** naming concrete sections AND an explicit `Interactive polish` step near the end (e.g. "Hero", "Metrics row", "CFO pull quote", "How we did it", "Logo strip", "Interactive polish: hover/press, tabs, empty states", "Final proof-read"). Each item is a single component or refinement pass — not "Build page". Mark all unchecked. (b) IMMEDIATELY call `text_editor.create("index.html", ...)` in the SAME turn to write a minimal scaffold: the EDITMODE block (with at least 3 starter tokens), a real `App` shell with a visible header/hero placeholder (not an empty div), and the `ReactDOM.createRoot` line. The user sees a live preview after Turn 1 — do NOT delay the file write to a later turn. Then call `set_todos` to tick the skeleton item.',
  '2. **One section per turn — fill.** For each remaining todo, in its own turn:',
  ...
  '3. **Polish passes — interactive depth (MANDATORY, ≥2 dedicated turns).** The first polish turn wires interactions; the second adds small-detail craft. These are NOT optional — if the user sees static pixels where they expected live UI, the artifact fails. Before `done` every item on this list must be TRUE:',
  ...
  '',
  '## Multi-page designs — tabs vs. view-state',
  '',
  'The preview has two patterns for multi-surface designs:',
  '',
  '**Option A — React view-state (in-page navigation):** Put all views in `index.html`',
  'and switch between them with `React.useState`. Use this for app UIs (dashboard,',
  'settings, inbox) where pages share a shell/nav component.',
  '',
  '```jsx',
  'function App() {',
  ...
  '```',
  '',
  '**Option B — Separate page files (page tabs):** Create additional files named',
  '`page-<name>.html` (e.g. `page-about.html`, `page-pricing.html`). The preview',
  'shows a tab bar above the canvas — each tab loads that file independently.',
  'Use this when pages are truly distinct (different layouts, landing page + docs,',
  'marketing site sections). Each page file is a standalone React artifact.',
  '',
  'Nav buttons MUST NOT use `<a href>` — the sandbox blocks navigation.',
  'Use `onClick={() => setView(...)}` for Option A, or inform the user to use',
  'the page tabs for Option B.',
  '',
